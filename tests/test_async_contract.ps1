...
$weaselMount = Read-ProjectFile 'bridge\src\weasel_mount.rs'
...
Assert-True ($bridge -match 'mod weasel_mount;' -and $bridge -match 'weasel_mount::install\(\)') 'bridge installs the runtime Weasel mount instead of requiring a patched server binary'
Assert-True ($bridge -match 'wake_target:\s*Option<weasel_mount::WakeTarget>' -and $bridge -match 'weasel_mount::capture_wake_target\(\)') 'each accepted async job captures its originating Weasel IPC session when available'
Assert-True ($bridge -match 'published_ready' -and $bridge -match 'weasel_mount::wake\(target\)') 'only a published ready completion can request an automatic Weasel wakeup'
...
Assert-True ($weaselMount -match 'hooked_get_status' -and $weaselMount -match 'probe\.is_composing' -and $weaselMount -match '_rime_bilingual_refresh') 'Rime refresh is injected only while upstream is reading a composing session status'
Assert-True ($weaselMount -match '\(hooks\.set_option\)\(session_id, option, next\)') 'mount triggers composition rebuild through the public librime option API'
Assert-True ($architecture -match 'DLL mount|DLL 掛載|runtime mount' -and $architecture -match '_rime_bilingual_refresh') 'architecture documents DLL-mounted completion wakeup and Rime refresh'
