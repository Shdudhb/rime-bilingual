...
filter 建立目前頁時，最多保存 20 個 genuine candidate reference、對應 immutable identity（absolute index/text/type/start/end）與 `local_hit`。每個真實鍵事件的 processor 仍可 bounded poll 保存 pair，但 UI 自動更新**不依賴下一個鍵事件**。bridge DLL 由 Lua 正常載入官方 Weasel process，runtime mount 先 pin exact `WeaselServer.exe` 與 `rime.dll` SHA-256，再在記憶體內觀察官方 named-pipe `ReadFile` request；不修改或替換磁碟上的 Weasel executable。filter 於真實 `PROCESS_KEY_EVENT` 內 submit 時保存 originating IPC session 與 foreground HWND。bridge worker 將 terminal `ready` completion 成功放入 completion ring 後，只有該 IPC session 仍 active、foreground 仍相同時才以 50 ms bounded `CallNamedPipeW` 發一個既有 `WEASEL_IPC_FOCUS_IN`，並帶 private `RBIL` marker；worker 不得直接呼叫 Rime API。若 focus 在 send 前後競態改變，ReadFile hook 會在 upstream dispatch 前把 wake 改寫成無副作用 `ECHO`。

官方 Weasel 處理 accepted `FOCUS_IN` 時會進入自己的 serialized `_UpdateUI(ipc_id)`。runtime mount 只包裝 pinned librime 1.13.1 的 `RimeApi.get_status` entry；當該 `_UpdateUI()` 第一次讀到對應 session status 時，wrapper 消耗 `RBIL` wake，先用原始 API 確認 `is_composing`，再以原始 `get_option` / `set_option` toggle schema-local `_rime_bilingual_refresh`。librime option update 因而重建未確認 composition，upstream `_UpdateUI()` 隨後繼續讀取新 context。filter 重跑後 `prepare_filter()` 先 poll matching ready result，讓英文 comment 在新 candidate yield 前寫入。pending/failed/stale/expired completion 不得發 completion wakeup；hash/ABI/session/focus/composing 任一不符都 fail closed。
...
本協定沒有 timer-driven polling：Lua 不 timer polling，也不送 synthetic key；bridge worker 也不呼叫 Rime API。唯一的主動喚醒是 terminal `ready` publication 後的 bounded official named-pipe transaction。Rime refresh 發生在官方 Weasel 自己的 serialized API handler 內，透過公開 `set_option` 路徑重建 composition；此 refresh 不執行 HTTP/SQLite/inference，只重建未確認 composition，filter 重跑後隨正常 UI render 顯示英文。若當下已失焦、session 不存在或不再 composing，mount 必須忽略或在 dispatch 前降級 wake。重啟 Weasel 即移除所有 runtime hook，官方 executable 檔案不需要復原。
